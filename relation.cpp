...
void relation::save(){
	...
	for (auto row : t){
		...
		for (auto attr : row.second){
			if (attr->get_class() == attr::attr_type::VAR_CHAR){
	...
	while (conj_iter != conjunctions.end()){
		...
		while (row_iter != t.end()){
			...
			if (this->meets_conjunction(*conj_iter, *row_iter)){
				std::vector<std::pair<std::string, attr*>> changes; //Stores the attribute-name of the attribute to change as well as the new attribute.
				...
				while (row_attr_iter < row_iter->second.size()){
					...
					if (row_iter->second[row_attr_iter]->get_class() == attr::attr_type::INTEGER){
					...
					if (this->is_key(this->header_pos(change_attr))){
						...
						for (int key_iter = 0; key_iter < row_iter->first.size(); ++key_iter){
							...
							if (key_iter != key_pos){
								...
								if (row_iter->first[key_iter]->get_class() == attr::attr_type::INTEGER){
								...
								if (change_iter->second->get_class() == row_iter->first[key_iter]->get_class()){
									...
									if (change_iter->second->get_class() == attr::attr_type::INTEGER){
				...
				if (key_changed){
					...
					while (row_key_iter < row_iter->first.size()){
						...
						if (row_iter->first[row_key_iter]->get_class() == attr::attr_type::INTEGER){
	...
	for (auto first_t : t){
		...
		for (auto second_t : other_table.t){
			...
			for (auto match : header_matches){
				attr* a1 = first_row[match.first];
				attr* a2 = second_row[match.second];
				if (*a1 == *a2){
					continue;
				}
			...
			if (matches){
				...
				for (int k = 0; k < first_row.size(); k++){
					...
					if (header_matches.find(k) == header_matches.end()){
						...
						if (first_row[k]->get_class() == attr::attr_type::INTEGER){
				...
				for (int k = 0; k < second_row.size(); k++){
					if (second_row[k]->get_class() == attr::attr_type::INTEGER){
